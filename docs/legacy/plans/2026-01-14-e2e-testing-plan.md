...
> Legacy note: this plan was written for a Neon branch-per-run strategy. That is no longer the default infrastructure direction for this template; keep it as historical context unless a future hosted-database migration intentionally revives that model.
