 ...
 *
 * The ledger cases below use a fake `events`; the D1 implementation
 * (`@gmacko/api`'s `WebhookEvents`) has its own suite
 * (`webhook-events.workers.test.ts`) and the cross-delivery behaviour under
 * injected faults is apps/web/fault/stripe-webhook.fault.ts.
 */

import type { WebhookClaim } from "@gmacko/api";
import { describe, expect, it } from "vitest";

import type { StripeWebhookLedger } from "../stripe-webhook";
import { handleStripeWebhook } from "../stripe-webhook";

/** A ledger that answers `claim` from a script and records `complete` calls. */
const fakeEvents = (
  claims: ReadonlyArray<WebhookClaim>,
): StripeWebhookLedger & { completed: Array<string> } => {
  const completed: Array<string> = [];
  let call = 0;
  return {
    completed,
    claim: async () => claims[call++] ?? "first",
    complete: async (id) => {
      completed.push(id);
    },
  };
...
describe("handleStripeWebhook", () => {
  ...
  it("runs the side effect once when Stripe redelivers the same event", async () => {
    const seen: Array<string> = [];
    const events = fakeEvents(["first", "duplicate"]);
    const signature = await sign(event, SECRET);
    for (let delivery = 0; delivery < 2; delivery += 1) {
      const response = await handleStripeWebhook(deliver(event, signature), {
        secret: SECRET,
        events,
        onEvent: (_type, id) => {
          seen.push(id);
      ...
      // Both deliveries are acknowledged: a 5xx would only make Stripe retry.
      expect(response.status).toBe(200);
    }
    expect(seen).toEqual(["evt_test_1"]);
    expect(events.completed).toEqual(["evt_test_1"]);
  });

  it("re-runs a claim whose previous delivery never completed", async () => {
    const seen: Array<string> = [];
    const events = fakeEvents(["retry"]);
    const response = await handleStripeWebhook(
      deliver(event, await sign(event, SECRET)),
      {
        secret: SECRET,
  ...
  it("does not acknowledge a delivery whose ledger write failed", async () => {
    const failing: StripeWebhookLedger = {
      claim: () => Promise.reject(new Error("D1 unavailable")),
      complete: () => Promise.resolve(),
    };
    await expect(
      handleStripeWebhook(deliver(event, await sign(event, SECRET)), {
        secret: SECRET,
        events: failing,
        onEvent: () => {
          throw new Error("the side effect must not run");
        },
      }),
