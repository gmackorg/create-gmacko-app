...
CLI + MCP wrappers over the same HTTP API (@gmacko/api-client).
...
Key scopes:
  ...
  delete  delete_post
  admin   create_api_key, revoke_api_key (key management needs an admin-scoped key)
  list_posts and get_post are public.

Notes:
  Use browser login in the app first, then create an API key for CLI or MCP automation.
  Flag values are strings; they are coerced through each tool's input schema
  (comma-separated lists, integers, true/false), which is derived from the API contract.
  Workspace context, billing overview, usage, and limits are exposed through the same API as the MCP server.
`;

async function main() {
  const argv = process.argv.slice(2);
  // Skip a leading `--` end-of-options separator (POSIX convention). pnpm
  // inserts one when forwarding `pnpm run api:ops -- <args>` through
