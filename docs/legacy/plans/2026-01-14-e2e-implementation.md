...
> Legacy note: this document assumes Neon branch-per-run isolation. The current template direction prefers colocated Postgres first and ForgeGraph-managed infrastructure, so the Neon-specific sections below should be treated as historical design context.
